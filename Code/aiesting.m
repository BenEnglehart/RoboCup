...
% while ~done
    ...
    [parray, nextstep, punisharray, num]=timestep({cpos},maxmove,erob,alyrob,tsreduc,passreduc,parray,nextstep,punisharray,0);
    [parray, nextstep, punisharray, num]=timestep(nextstep,maxmove,erob,alyrob,tsreduc,passreduc,parray,nextstep,punisharray,num);
    %[parray, nextstep, punisharray, num]=timestep(nextstep,maxmove,erob,alyrob,tsreduc,passreduc,parray,nextstep,punisharray,num);
    disp(parray(1))
...
% end
function [parr, nstep, punisharray, count]=timestep(cpos,maxmove,erob,alyrob,tsreduc,passreduc,parr,nstep,punisharray,count)
% disp(count);
for ll=count+1:length(cpos)
    ...
    anewposx=[];
    anewposy=[];
    for ii=-maxmove:1:maxmove
        for jj=-maxmove:1:maxmove
            newposx(end+1)=pos(1)+jj;
            newposy(end+1)=pos(2)+ii;
        end
    end
    ...
    if length(cpos)>1
        for ii=-maxmove:1:maxmove
            for jj=-maxmove:1:maxmove
                anewposx(end+1)=alyrob(1)+jj;
                anewposy(end+1)=alyrob(2)+ii;
            end
        end
    end
